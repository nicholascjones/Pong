...
#define PV 80.00 //paddle velocity is 5.00 pixels per STEP, or 50 pixels (one paddle) per second
...
void EndGame(int,int[3]); //displays end of game screen
...
int main()
{
	...
	char Menu[6][50]; //displays strings in menu
	int q = 0;
	int b = 0;
	int w = 0; //winner at end of game
	...
	strcpy(Menu[3],"To quit ... Press q");
	strcpy(Menu[4],"The s (up)  and x (down) keys control your paddle");
	strcpy(Menu[5],"Please enter a valid input.");
	...
	for (i = 0; i < 5; i++) {
		//color
		switch (i) {
			case (1): gfx_color(255,255,0);
				  break;
			case (2): gfx_color(255,0,0);
				  break;
			case (3): gfx_color(0,255,0);
				  break;
			case (0): gfx_color(0,255,255);
				  break;
			default:  gfx_color(255,255,255);
		}
		gfx_text(XSIZE/3,(i+1)*YSIZE/7,Menu[i]);
		gfx_flush();
	}

	c = gfx_wait(); //takes input
	while ((c != 'e') && (c != 'h') && (c != 'q')) { //error case
		gfx_color(255,255,255);
		gfx_text((2*XSIZE/3)-30,YSIZE/2,Menu[5]);
		c = gfx_wait(); //takes input
	}
	...
	switch (w) {
		case (1): EndGame(1,score);
			  break;
		case (2): EndGame(2,score);
			  break;
		default:  break;
	} //ends game			
	}
...
int Initialize( float x[3], float y[3], float v[2], int q)
{
	...
	for (i = 0; i <= 1; i++) {
		...
		r = (((float)rand()-(RAND_MAX/2))/(RAND_MAX/2)); //random proportion from -1 to 1
		if(r > 0)
			v[i] = (r*PV)+PV/2; //random proportion of 5.00
		else
			v[i] = (r*PV)-PV/2; //random proportion of 5.00
...
void Recalculate(float* xPtr, float y[3], float v[2], int b, char lvl)
{
	...
	*xPtr = (float)*xPtr + v[0]*STEP;
	...
	//paddles
        srand( (float)time(&t)+5261.0*(float)b);
	switch (lvl) {
			case ('h'): r = (rand()-(RAND_MAX/2))/(float)(RAND_MAX/2)*75.00; //random value between 1 and 50	
				    break;
			case ('e'): r = (rand()-(RAND_MAX/2))/(float)(RAND_MAX/2)*200.00; //random value between 1 and 50
				    break;
	}
				    y[2] = y[0]-50.00+r;
	
	if(y[1] < 0) y[1] = 0;
	else if(y[1] > YSIZE - HEIGHT) y[1] = YSIZE - HEIGHT;

        if(y[2] < 0) y[2] = 0;
	else if(y[2] > YSIZE - HEIGHT) y[2] = YSIZE - HEIGHT;
}
...
//ends game
void EndGame(int w, int score[3])
{
	char c; //user hits character
	char sayings[7][50]; //sayings
	int x = 1; //
...
while (x == 1) {
	...
	strcpy(sayings[0],"Press Any Button To Exit");
	strcpy(sayings[1],"You won!!!");
	strcpy(sayings[2],"You lost :\(");
	strcpy(sayings[3],"Awesome job!!");
	strcpy(sayings[4],"Better luck next time...");
//	strcpy(sayings[5],num2str(score[1]),sayings[5]);
//	strcpy(sayings[6],num2str(score[2]),sayings[6;
	

	switch (w) {
		case (1): gfx_color(150,150,255); 
			  gfx_text(XSIZE/3,(YSIZE/3),sayings[1]);
			  gfx_text(XSIZE/3,(YSIZE/3)+20,sayings[3]);
			//  gfx_text(XSIZE/3,(YSIZE/3)-20,"%s - %s",sayings[5],sayings[6]
			  break;
		case (2): gfx_color(255,0,0);
                          gfx_text(XSIZE/3,(YSIZE/3),sayings[2]);
                          gfx_text(XSIZE/3,(YSIZE/3)+20,sayings[4]);
			  break;
	}
	
	gfx_color(255,255,255);
	gfx_text(XSIZE/3,(2*YSIZE/3),sayings[0]);

	if (gfx_event_waiting()) x = 0; 
}
}
